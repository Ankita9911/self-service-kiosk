...
export function OutletManagerView({
  data,
  visibleIds,
  loading = false,
  period,
    ...
    "om-summary": () => (
      <section>
        <SectionTitle>
          ...
          Outlet Performance — {periodLabel}
        </SectionTitle>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          ...
          {isToday && (
            <MetricCard
              loading={loading}
              label="Peak Hour"
              value={formatHour(summary.peakHour)}
              sub="Busiest today"
      ...
      <section>
        ...
        >
          {isToday ? (
            ...
            <EmptyState
              message={`No revenue data for ${periodLabel.toLowerCase()}.`}
            />
          )}
        </WidgetCard>
      </section>
    ...
    "om-top-items": () => (
      <section className="flex flex-col">
        <SectionTitle>
          ...
          Top 5 Items — {periodLabel}
        </SectionTitle>
        <WidgetCard
          title="Best Sellers"
          subtitle={`Most sold items — ${periodLabel.toLowerCase()}`}
          noPad
          loading={loading}
          loadingHeight="h-[238px]"
          className="flex-1"
        >
          {topItems.length > 0 ? (
    ...
    "om-category": () => (
      <section className="flex flex-col">
        <SectionTitle>
          ...
          Category Revenue — {periodLabel}
        </SectionTitle>
        <WidgetCard
          title="Revenue by Category"
          subtitle="Which categories drive the most sales"
          loading={loading}
          loadingHeight="h-[238px]"
          className="flex-1"
        >
  ...
  // Render in visibleIds order; pair om-top-items + om-category side-by-side when adjacent
  const SIDE_PAIR = new Set(["om-top-items", "om-category"]);
