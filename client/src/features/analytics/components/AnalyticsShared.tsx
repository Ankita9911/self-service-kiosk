...
type Accent = "indigo" | "emerald" | "blue" | "violet" | "rose" | "amber" | "slate";

const ACCENT_MAP: Record<Accent, { iconBg: string; trendPos: string; trendNeg: string }> = {
  indigo:  { iconBg: "bg-indigo-50 dark:bg-indigo-500/10",   trendPos: "text-emerald-500 dark:text-emerald-400", trendNeg: "text-red-500 dark:text-red-400" },
  emerald: { iconBg: "bg-emerald-50 dark:bg-emerald-500/10", trendPos: "text-emerald-500 dark:text-emerald-400", trendNeg: "text-red-500 dark:text-red-400" },
  blue:    { iconBg: "bg-blue-50 dark:bg-blue-500/10",       trendPos: "text-emerald-500 dark:text-emerald-400", trendNeg: "text-red-500 dark:text-red-400" },
  violet:  { iconBg: "bg-violet-50 dark:bg-violet-500/10",   trendPos: "text-emerald-500 dark:text-emerald-400", trendNeg: "text-red-500 dark:text-red-400" },
  rose:    { iconBg: "bg-rose-50 dark:bg-rose-500/10",       trendPos: "text-emerald-500 dark:text-emerald-400", trendNeg: "text-red-500 dark:text-red-400" },
  amber:   { iconBg: "bg-amber-50 dark:bg-amber-500/10",     trendPos: "text-emerald-500 dark:text-emerald-400", trendNeg: "text-red-500 dark:text-red-400" },
  slate:   { iconBg: "bg-slate-50 dark:bg-white/[0.05]",     trendPos: "text-emerald-500 dark:text-emerald-400", trendNeg: "text-red-500 dark:text-red-400" },
};

function normalizeAccent(accent: string): Accent {
  const map: Record<string, Accent> = {
    orange: "indigo", // remap legacy orange → indigo
    indigo: "indigo", emerald: "emerald", blue: "blue",
    violet: "violet", rose: "rose", amber: "amber", slate: "slate",
  };
  return map[accent] ?? "indigo";
}

export function MetricCard({
  label, value, sub, icon, accent = "indigo", trend, loading = false,
}: {
  label: string;
  value: string | number;
  sub?: string;
        ...
        <div className="mt-3 pt-3 border-t border-slate-50 dark:border-white/5 flex items-center gap-1.5 flex-wrap">
          ...
          {trend !== undefined && (
            <span className={`text-[11px] font-semibold ml-auto ${isPositive ? a.trendPos : a.trendNeg}`}>
              {isPositive ? "↑" : "↓"} {Math.abs(trend.value)}%{trend.label ? ` ${trend.label}` : ""}
            </span>
          )}
        </div>
      )}
...
export function WidgetCard({
  ...
  className = "", noPad = false,
  loading = false, loadingHeight = "h-[220px]",
}: {
  title?: string;
  subtitle?: string;
...
export function TopItemsList({
  items, loading = false, count = 5,
}: {
  items: Array<{ _id: unknown; name: string; totalSold: number; totalRevenue: number }>;
  loading?: boolean;
  count?: number;
}) {
  const rankColors = [
    ...
    <div className="divide-y divide-slate-50 dark:divide-white/4">
      {items.map((item, i) => (
        <div key={String(item._id)} className="flex items-center gap-3 py-2.5 px-5 hover:bg-slate-50/50 dark:hover:bg-white/2 transition">
          ...
          <span className="flex-1 text-[12.5px] text-slate-700 dark:text-slate-300 truncate">{item.name}</span>
          <span className="text-[11px] text-slate-400 dark:text-slate-500 shrink-0">{item.totalSold} sold</span>
...
export function OutletProgressList({
  outlets, loading = false, count = 4,
}: {
  outlets: Array<{ outletId: unknown; name?: string; outletCode: string; revenue: number; orders: number; contributionPercent: number }>;
  loading?: boolean;
  count?: number;
}) {
  if (loading) {
        ...
        <div key={String(o.outletId)}>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div className="flex items-center gap-2 shrink-0 ml-2">
              <span className="text-[11px] text-slate-400 dark:text-slate-500">{o.orders} orders</span>
...
const STATUS_BADGE_MAP: Record<string, string> = {
  CREATED:    "bg-slate-100 text-slate-500 dark:bg-white/[0.07] dark:text-slate-400",
  IN_KITCHEN: "bg-amber-50 text-amber-600 dark:bg-amber-500/10 dark:text-amber-400",
