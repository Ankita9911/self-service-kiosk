...
function HealthCard({
      ...
      <div className="bg-white dark:bg-[#161920] rounded-2xl border border-slate-100 dark:border-white/6 p-5">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="space-y-1.5 flex-1">
            ...
            <AnalyticsShimmer className="h-3 w-16" />
          </div>
          <div className="space-y-1.5">
            <AnalyticsShimmer className="h-3.5 w-16" />
            <AnalyticsShimmer className="h-3 w-12" />
          </div>
        </div>
  ...
  return (
    <div className="bg-white dark:bg-[#161920] rounded-2xl border border-slate-100 dark:border-white/6 p-5">
      <div className="flex items-center gap-3 mb-4">
        <div
          className={`h-9 w-9 rounded-xl ${color} flex items-center justify-center shrink-0`}
        >
          {icon}
        </div>
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-5 hover:bg-slate-50/50 dark:hover:bg-white/2 transition">
      ...
      <div className="flex flex-col items-end gap-1 shrink-0">
        <span
          className={`inline-flex items-center text-[10.5px] font-medium px-2 py-0.5 rounded-full ${
            isActive
...
export function SuperAdminView({
  ...
  const widgetRenderers: Record<string, () => React.ReactNode> = {
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <HealthCard
            loading={loading}
            label="Franchises"
            active={summary.activeFranchises}
            total={summary.totalFranchises}
            icon={<Building2 className="w-4 h-4 text-indigo-500" />}
    ...
    "sa-top-franchises": () => (
      <section>
        <SectionTitle>
          ...
          Top 5 Franchises by Outlets
        </SectionTitle>
        <WidgetCard loading={loading} loadingHeight="h-[220px]">
          {outletsByFranchise.length > 0 ? (
            <CountBarChart
              data={outletsByFranchise.map((f) => ({
                name: f.name || f.brandCode,
                count: f.outletCount,
              }))}
              label="Outlets"
              height={220}
    ...
    "sa-top-devices": () => (
      <section>
        <SectionTitle>
          ...
          Top 5 Outlets by Devices
        </SectionTitle>
        <WidgetCard loading={loading} loadingHeight="h-[220px]">
          {devicesByOutlet.length > 0 ? (
            <CountBarChart
              data={devicesByOutlet.map((o) => ({
                name: o.name || o.outletCode,
                count: o.deviceCount,
              }))}
              label="Devices"
              height={220}
    ...
    "sa-roles": () => (
      <section>
        ...
        <WidgetCard
          title="Roles"
          subtitle="All users across the platform grouped by role"
          loading={loading}
          loadingHeight="h-[220px]"
        >
  ...
  for (let i = 0; i < visibleIds.length; i++) {
    ...
    if (
      SIDE_PAIR.has(id) &&
      nextId &&
      SIDE_PAIR.has(nextId) &&
      !processed.has(nextId)
    ) {
      widgets.push(
        <div
          key={`${id}+${nextId}`}
