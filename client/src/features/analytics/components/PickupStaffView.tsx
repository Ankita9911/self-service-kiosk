...
export function PickupStaffView({ data, visibleIds, loading = false }: Props) {
  ...
  return (
    <div className="space-y-5">
      ...
      {show("ps-summary") && (
        <section>
          <SectionTitle><ShoppingBag className="w-3.5 h-3.5 text-indigo-500" />Pickup Counter</SectionTitle>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <MetricCard loading={loading} label="Ready for Pickup"  value={readyCount}      sub="Awaiting collection"  icon={<Package     className="w-4 h-4 text-indigo-500"  />} accent="indigo"  />
            <MetricCard loading={loading} label="Handed Over Today" value={handedOverToday} sub="Picked up orders"     icon={<CheckCircle className="w-4 h-4 text-emerald-500" />} accent="emerald" />
            ...
            <MetricCard loading={loading} label="Avg Pickup Delay"  value={avgPickupDelayMinutes !== null ? `${avgPickupDelayMinutes} min` : "—"} sub="From READY to pickup" icon={<Clock className="w-4 h-4 text-amber-500" />} accent="amber" />
          </div>
        </section>
      )}
      ...
      {show("ps-ready") && (
        <section>
          <SectionTitle><Package className="w-3.5 h-3.5 text-indigo-500" />Orders Ready for Pickup</SectionTitle>
          <WidgetCard title="Ready Queue" subtitle="Awaiting customer collection" loading={loading} loadingHeight="h-[160px]">
            {readyOrders.length > 0 ? (
              <div className="space-y-2">
                {readyOrders.map((order) => (
                  <div key={order._id} className="flex items-center justify-between px-4 py-3 rounded-xl bg-indigo-50 dark:bg-indigo-500/[0.08] border border-indigo-100 dark:border-indigo-500/20">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[13px] font-medium text-slate-700 dark:text-slate-200">Order #{order.orderNumber}</span>
                    </div>
              ...
              <div className="flex items-center gap-3 py-4">
                ...
                <p className="text-[13px] text-slate-500 dark:text-slate-400">No orders waiting for pickup.</p>
              </div>
            )}
          </WidgetCard>
        </section>
