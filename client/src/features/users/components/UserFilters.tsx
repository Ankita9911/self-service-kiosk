...
const ROLE_ICONS: Record<string, ComponentType<{ className?: string }>> = {
  ALL: Users,
  FRANCHISE_ADMIN: ShieldCheck,
  OUTLET_MANAGER: UserCog,
...
export function UserFilters({
  ...
  const selectedRole =
    ALL_ROLES.find((role) => role.value === roleFilter) ?? ALL_ROLES[0];
  const SelectedRoleIcon = ROLE_ICONS[selectedRole.value] ?? Users;
      ...
      >
        <SelectTrigger className="h-9 w-40 rounded-xl border-slate-100 dark:border-white/8 bg-white dark:bg-[#161920] text-[13px] text-slate-700 dark:text-slate-200 focus:ring-indigo-400/20">
          <div className="flex items-center gap-2 truncate">
            <SelectedRoleIcon className="w-3.5 h-3.5 text-slate-500 dark:text-slate-400 shrink-0" />
        ...
        <SelectContent className="rounded-xl border-slate-100 dark:border-white/8 bg-white dark:bg-[#1a1d26]">
          {ALL_ROLES.map((r) => {
            const RoleIcon = ROLE_ICONS[r.value] ?? Users;
            return (
              <SelectItem
                key={r.value}
                value={r.value}
