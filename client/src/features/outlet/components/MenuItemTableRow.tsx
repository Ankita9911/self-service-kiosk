...
export function MenuItemTableRow({ item, categories, index, onEdit, onDelete, onUpdatePrice, onUpdateStock, onToggleStatus, onView }: Props) {
  ...
  return (
    <>
    <tr className={`group border-b border-slate-100 dark:border-white/5 transition-colors hover:bg-slate-50/70 dark:hover:bg-white/2 ${
      index % 2 === 0 ? "" : "bg-slate-50/30 dark:bg-white/1"
    }`}>
      {/* # */}
      <td className="px-4 py-3 w-10">
        <span className="text-xs text-slate-400 dark:text-slate-600 font-mono">{index}</span>
      </td>
      ...
      <td className="px-4 py-3 text-right">
        <div className="relative inline-block" ref={menuRef}>
          ...
          {menuOpen && (
            <div className={`absolute right-0 ${openAbove ? "bottom-9" : "top-9"} z-50 w-44 bg-white dark:bg-[#1e2130] border border-slate-100 dark:border-white/8 rounded-xl shadow-lg shadow-black/10 dark:shadow-black/40 py-1 overflow-hidden`}>
              {menuActions.map(({ icon: Icon, label, onClick, className }) => (
                <button
                  key={label}
