...
const inputBase =
  "w-full px-3 h-10 rounded-xl border bg-slate-50 dark:bg-white/5 text-sm text-slate-800 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-600 focus:outline-none focus:ring-2 transition-all border-slate-200 dark:border-white/8 focus:ring-indigo-500/30 focus:border-indigo-400";
...
export function EditComboModal({
  ...
  useEffect(() => {
    ...
    const initialItems: ComboItemEntry[] = (combo.items ?? []).map(
      (ci: any) => {
  ...
  function addItem() {
    ...
    setSelectedItems((prev) => {
      ...
      if (existing)
        return prev.map((e) =>
          e.menuItemId === found._id ? { ...e, quantity: e.quantity + 1 } : e,
        );
      return [
        ...prev,
        {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg p-0 border border-slate-100 dark:border-white/8 bg-white dark:bg-[#1e2130] rounded-2xl overflow-hidden shadow-2xl shadow-black/20">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">
                Update combo details and pricing
              </p>
            </div>
          </div>
          <button
        ...
        >
          <div>
            <LabelEl>
              Combo Name <span className="text-red-400 normal-case">*</span>
            </LabelEl>
            <input
              value={name}
                ...
                <button
                  ...
                  className={`flex-1 h-9 rounded-xl border text-xs font-semibold transition-all ${
                    serviceType === opt.value
                      ...
                      : "border-slate-200 dark:border-white/8 text-slate-500 dark:text-slate-400"
                  }`}
                >
                  {opt.label}
                </button>
              ...
              <div className="mt-2 space-y-2">
                {selectedItems.map((entry) => (
                  <div
                    ...
                    className="flex items-center justify-between bg-slate-50 dark:bg-white/5 rounded-xl px-3 py-2 border border-slate-100 dark:border-white/8"
                  >
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-sm font-bold text-slate-700 dark:text-white min-w-5 text-center">
                        {entry.quantity}
                      </span>
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        className="ml-1 w-6 h-6 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-400/10 transition-colors"
                      >
                        <X className="w-3 h-3" />
                      </button>
                    </div>
          ...
          <div>
            ...
            {comboPrice &&
              originalPrice > 0 &&
              parseFloat(comboPrice) < originalPrice && (
                <p className="mt-1 text-[11px] text-emerald-600 dark:text-emerald-400 font-semibold">
                  Customer saves ₹
                  {(originalPrice - parseFloat(comboPrice)).toFixed(2)}
                </p>
              )}
          </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <Layers className="w-4 h-4" /> Save Changes
                </>
              )}
            </button>
          </div>
