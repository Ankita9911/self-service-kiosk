...
export function CreateComboModal({ open, onClose, items, onSubmit }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setName(""); setDescription(""); setComboPrice(""); setSelectedItems([]); setServiceType("BOTH");
      onClose();
    } finally {
      ...
      <DialogContent className="sm:max-w-lg p-0 border border-slate-100 dark:border-white/8 bg-white dark:bg-[#1e2130] rounded-2xl overflow-hidden shadow-2xl shadow-black/20">
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100 dark:border-white/8">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800 dark:text-white">Create Combo</h3>
              <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">Bundle items together at a special price</p>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" disabled={isSubmitting || !name.trim() || !comboPrice || selectedItems.length === 0}
              ...
              {isSubmitting ? <RefreshCcw className="w-4 h-4 animate-spin" /> : <><Layers className="w-4 h-4" /> Create Combo</>}
            </button>
          </div>
        </form>
      </DialogContent>
