...
export function IngredientRowMenu({ ingredient, onEdit, onDelete, onAdjustStock }: Props) {
    ...
    return (
        <div className="relative" ref={ref}>
            ...
            {menuOpen && (
                ...
                >
                    <button
                        onClick={() => { setMenuOpen(false); onEdit(ingredient); }}
                    ...
                    <button
                        onClick={() => { setMenuOpen(false); onAdjustStock(ingredient); }}
                        className="w-full flex items-center gap-3 px-4 py-2.5 text-[13px] text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-white/4 transition-colors"
                    >
                        <ArrowUpDown className="w-3.5 h-3.5 text-emerald-500" /> Adjust Stock
                    </button>
                ...
                <Modal onClose={!busy ? close : undefined}>
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-white/6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-[13px] font-semibold text-slate-900 dark:text-white">Delete Ingredient</h3>
                                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 truncate max-w-40">{ingredient.name}</p>
                            </div>
                        </div>
                        {!busy && (
                    ...
                    <div className="px-5 py-4">
                        <p className="text-[13px] text-slate-500 dark:text-slate-400 leading-relaxed">
                            This will permanently delete <strong className="text-slate-700 dark:text-slate-200">{ingredient.name}</strong>. This action cannot be undone.
                        </p>
                    </div>
