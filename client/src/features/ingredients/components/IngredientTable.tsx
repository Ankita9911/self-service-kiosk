...
export function IngredientTable({ ingredients, loading, onEdit, onDelete, onAdjustStock }: Props) {
  ...
  return (
    <div className="bg-white dark:bg-[#1e2130] rounded-2xl border border-slate-100 dark:border-white/[0.06] overflow-hidden">
              ...
              <tr key={ing._id} className="border-b border-slate-50 dark:border-white/[0.04] hover:bg-slate-50/60 dark:hover:bg-white/[0.02] transition-colors">
                ...
                <td className="px-5 py-3.5">
                  <div className="flex items-center justify-end gap-1">
                    <Button variant="ghost" size="icon" className="h-8 w-8" title="Adjust Stock" onClick={() => onAdjustStock(ing)}>
                      <Package className="w-3.5 h-3.5 text-blue-600 dark:text-blue-400" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8" title="Edit" onClick={() => onEdit(ing)}>
                      <Pencil className="w-3.5 h-3.5" />
                    </Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8" title="Delete" onClick={() => onDelete(ing._id)}>
