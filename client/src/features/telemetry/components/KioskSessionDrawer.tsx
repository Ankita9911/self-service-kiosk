...
export function KioskSessionDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[120] flex justify-end bg-slate-950/45 backdrop-blur-sm">
      ...
      <aside className="relative h-full w-full max-w-2xl overflow-y-auto border-l border-white/10 bg-[#0d1117] text-slate-100 shadow-2xl">
        <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-white/10 bg-[#0d1117]/95 px-6 py-5 backdrop-blur">
          <div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border border-white/10 text-slate-300 transition hover:border-white/20 hover:text-white"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <div className="space-y-6 px-6 py-6">
          {loading ? (
            ...
            <div className="rounded-[24px] border border-rose-500/30 bg-rose-500/10 p-5 text-sm text-rose-200">
              {error}
            </div>
          ) : (
            <>
              ...
              <section className="rounded-[24px] border border-white/10 bg-white/[0.03] p-5">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="rounded-2xl bg-white/[0.05] px-3 py-2 text-sm text-slate-300">
                  ...
                  {events?.items.length ? (
                    events.items.map((event) => (
                      ...
                      >
                        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                          <div>
                            ...
                            <p className="mt-1 text-xs text-slate-400">
                              {event.page || "unknown"} / {event.component || "unknown"} /{" "}
                        ...
                        <p className="mt-3 break-all rounded-xl bg-white/[0.04] px-3 py-2 font-mono text-[11px] text-slate-300">
