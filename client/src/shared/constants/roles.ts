...
export const ROLE_ICONS: Record<string, React.ReactNode> = {
  ...
  FRANCHISE_ADMIN: <Shield className="w-3 h-3" />,
  OUTLET_MANAGER: <Store className="w-3 h-3" />,
  KITCHEN_STAFF: <ChefHat className="w-3 h-3" />,
  PICKUP_STAFF: <PackageCheck className="w-3 h-3" />,
};
...
export const DEFAULT_ROLE_BADGE_STYLE =
...
export function getRoleIcon(role?: string): React.ReactNode {
  return role && ROLE_ICONS[role] ? ROLE_ICONS[role] : <Shield className="w-3 h-3" />;
}
